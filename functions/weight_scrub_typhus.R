...
output_dir <- "./outputs/scrub_typhus_weights/"
dir.create(output_dir, showWarnings = FALSE, recursive = TRUE)

# -------------------- 2. Data Preparation --------------------
data <- read_csv("./data/soc_data/scrub_typhus_soc.csv") %>%
  mutate(log_gdp = log(gdp + 1),
         log_pop = log(pop_count + 1)) %>%
  drop_na()

var_name <- c("log_gdp", "log_pop", "urban_accessibility", "walking_only_travel_time_to_healthcare", "cases", "XX", "XY")
data_model <- data[, c("id", var_name)]
...
for (i in 1:n_iter) {
  ...
  glm_formula <- as.formula("if_survey ~ log_gdp + log_pop + urban_accessibility + walking_only_travel_time_to_healthcare")
  glm <- tryCatch({
  ...
  # Save model
  model_file <- file.path(output_dir, paste0("bayesglm_model_", i, ".rds"))
  ...
  survey_scaled[, c("log_gdp", "log_pop", "urban_accessibility", "walking_only_travel_time_to_healthcare")] <-
    scale(survey_scaled[, c("log_gdp", "log_pop", "urban_accessibility", "walking_only_travel_time_to_healthcare")])
  glm_scaled <- tryCatch({
...
write_csv(positive, file.path(output_dir, "scrub_typhus_weight_positive.csv"))
write_csv(data_model, file.path(output_dir, "scrub_typhus_weight_all.csv"))
